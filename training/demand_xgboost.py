...
for fold, (train_idx, val_idx) in enumerate(tscv.split(X)):
    ...
    # CHANGE: Use XGBoost instead of LightGBM
    ...
    # CHANGE: XGBoost eval_set format
    model.fit(
        X_train, y_train,
...
plt.tight_layout()
# plt.savefig("images/xgb_cv_results.png")
print("📁 Cross-validation results saved to xgb_cv_results.png")

# Set up XGBoost hyperparameters
print("\n🔍 Using pre-defined hyperparameters for XGBoost...")
best_params = {
    'learning_rate': 0.03,
    'max_depth': 8,
    'min_child_weight': 5,
...
# CHANGE: XGBoost fit format
model.fit(
    X_train_selected, y_train,
    eval_set=[(X_test_selected, y_test)],
    verbose=100  # Keep this for progress updates
)
...
# CHANGE: XGBoost feature importance extraction
...
plt.title("XGBoost Feature Importances (Log Demand)")
plt.tight_layout()
# plt.savefig("images/xgb_feature_importances_log.png")
print("📁 Feature importances saved to xgb_feature_importances_log.png")

# Add plotting code for actual vs. predicted quantities
...
plt.title(f'XGBoost: Actual vs. Predicted Quantities (R² = {r2:.2f})')
plt.legend()
...
# plt.savefig("images/xgb_actual_vs_predicted.png")
print("📁 Actual vs. predicted plot saved to xgb_actual_vs_predicted.png")
...
# plt.savefig("images/xgb_residuals.png")
print("📁 Residual plot saved to xgb_residuals.png")
...
bin_errors = comparison_df.groupby('Quantity_Bin', observed=False)[['Actual', 'Predicted']].apply(
...
plt.title('XGBoost: Prediction Error by Quantity Range')
plt.grid(True, axis='y', alpha=0.3)
plt.tight_layout()
# plt.savefig("images/xgb_prediction_error_by_range.png")
print("📁 Prediction error by range plot saved to xgb_prediction_error_by_range.png")
...
plt.ylabel('Predicted Quantity')
plt.title('XGBoost: Price Sensitivity Analysis (Smoothed)')
plt.legend()
plt.grid(True, alpha=0.3)
plt.tight_layout()
# plt.savefig("images/xgb_price_sensitivity_smoothed.png")
print("📁 Smoothed price sensitivity analysis saved to xgb_price_sensitivity_smoothed.png")
...
plt.title('XGBoost: Total Predicted Demand per Week (Aggregated Over All Years)')
plt.xlabel('Week Number')
plt.ylabel('Summed Predicted Quantity')
plt.grid(True, alpha=0.3)
plt.tight_layout()
# plt.savefig("images/xgb_predicted_demand_summed_by_week.png")
print("📁 Weekly demand plot saved to xgb_predicted_demand_summed_by_week.png")

# Product-specific price elasticity analysis
...
if sum(product_mask) < 10:
    ...
    plt.title(f'XGBoost: Price Sensitivity for Product {product_id}')
    plt.xlabel('Price Multiplier')
    ...
    plt.tight_layout()
    # plt.savefig(f"product_elasticity/xgb_product_{product_id}_elasticity.png")
    print(f"📁 Product elasticity plot saved to product_elasticity/xgb_product_{product_id}_elasticity.png")

# Create a directory for product images if it doesn't exist
import os
os.makedirs("product_elasticity", exist_ok=True)

# Compare LightGBM vs XGBoost performance
try:
    lgbm_model = joblib.load('../models/lgbm_model.pkl')
    
    print("\n🔍 Comparing XGBoost vs LightGBM performance...")
    
    # Get predictions from both models on test data
    lgbm_pred = np.expm1(lgbm_model.predict(X_test_selected))
    xgb_pred = y_pred  # We already have this from earlier
    ...
    # Print comparison
    print("\n📊 Model Comparison:")
    print(f"{'Metric':<10} {'LightGBM':>10} {'XGBoost':>10} {'Difference':>12}")
    ...
    print("📁 Model comparison saved to images/model_comparison.png")
    
except Exception as e:
    print(f"⚠️ Could not compare with LightGBM: {e}")

print("\n✅ XGBoost demand prediction complete!")